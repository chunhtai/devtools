...
class _DeepLinkListViewState extends State<DeepLinkListView>
  ...
  void initState() {
    ...
    callWhenControllerReady((_) {
      ...
      controller.selectedVariantIndex = releaseVariantIndex;
    });
  }
    ...
    return Row(
      ...
      children: [
        Padding(
          ...
          child: _buildAndroidVariantDropDown(),
        ),
      ],
    );
  }

  Widget _buildAndroidVariantDropDown() {
    final List<DropdownMenuEntry<int>> colorEntries =
        <DropdownMenuEntry<int>>[];
    for (int i = 0; i < androidVariants.length; i += 1) {
      colorEntries.add(
        DropdownMenuEntry<int>(
          value: i,
          label: androidVariants[i],
        ),
      );
    }
    return DropdownMenu<int>(
      initialSelection: controller.selectedVariantIndex,
      label: const Text('Android Variant'),
      dropdownMenuEntries: colorEntries,
      onSelected: (int? index) {
        setState(() {
          controller.selectedVariantIndex = index!;
        });
      },
    );
  }
